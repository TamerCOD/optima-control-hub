...
const App: React.FC = () => {
  ...
  }, [currentUser, currentView, isAuthChecking]);
        ...
        db.collection('users').doc(firebaseUser.uid).onSnapshot((doc) => {
            if (doc.exists) {
                ...
                if (!data.isDeleted) {
                    ...
                    // User exists but is marked as deleted
                    setCurrentUser(null);
                    auth.signOut();
                    alert('Ваша учетная запись была удалена или деактивирована.');
                }
            } else {
  ...
  // Creates user using Admin API (server-side)
  ...
  const handleDeleteUser = async (uid: string) => {
      try {
          ...
          alert('Сотрудник полностью удален из системы.');
      } catch (e: any) {
    ...
    <ErrorBoundary>
        ...
        >
            <Suspense fallback={<RocketLoader text="Синхронизация..." />}>
                ...
                {currentView === 'USERS' && <UserManagement users={users} departments={departments} roles={roles} currentUser={currentUser} onAddUser={handleAddUser} onUpdateUser={u => db.collection('users').doc(u.id).update(cleanPayload(u))} onDeleteUser={handleDeleteUser} onCreateRole={r => db.collection('roles').doc(r.id).set(cleanPayload(r))} />}
