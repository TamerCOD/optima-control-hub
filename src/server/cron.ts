...
export async function runAllCronJobs(): Promise<string[]> {
  ...
  try {
    ...
    try {
      const settingsDoc = await db.collection('settings').doc('tms').get();
    ...
    try {
      ...
      const issueResult = await runIssueAutomation();
...
async function checkSyncDepNotifications(): Promise<string[]> {
  const logs: string[] = [];
  try {
    const settingsDoc = await db.collection('settings').doc('sync_dep_config').get();
    if (!settingsDoc.exists) {
