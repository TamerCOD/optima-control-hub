...
import { ProjectTask, User, Project, TaskSettings, TaskComment, TaskUpdateEntry } from '../../types';
        ...
        <div className="flex flex-col h-full gap-4">
            <div className="flex justify-between px-4 items-center">
                <div className="flex items-center gap-2 bg-zinc-100 dark:bg-zinc-800 p-1.5 rounded-2xl border border-zinc-200 dark:border-zinc-700">
                    <button onClick={() => setGroupBy('status')} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all ${groupBy === 'status' ? 'bg-white dark:bg-zinc-900 text-primary shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>Статус</button>
                    <button onClick={() => setGroupBy('project')} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all ${groupBy === 'project' ? 'bg-white dark:bg-zinc-900 text-primary shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>Проект</button>
                    <button onClick={() => setGroupBy('priority')} className={`px-4 py-2 rounded-xl text-[10px] font-bold uppercase transition-all ${groupBy === 'priority' ? 'bg-white dark:bg-zinc-900 text-primary shadow-sm' : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'}`}>Приоритет</button>
                </div>
                <button 
                    onClick={() => setShowCompleted(!showCompleted)}
                    className={`btn-3d flex items-center gap-2 px-5 py-2.5 text-[10px] font-bold uppercase tracking-widest ${showCompleted ? 'bg-primary text-white' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 border border-zinc-200 dark:border-zinc-700'}`}
                >
                ...
                {columns.map(col => {
                    ...
                    return (
                        <div 
                            ...
                            className="flex flex-col w-[340px] shrink-0 bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-3xl overflow-hidden shadow-sm"
                        >
                            <div className="p-3 flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800">
                                ...
                                {columnTasks.map(task => {
                                    ...
                                    return (
                                        <div 
                                            ...
                                            className={`bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 p-3 rounded-2xl hover:border-primary/50 transition-all cursor-pointer group animate-fade-in shadow-sm ${draggedTaskId === task.id ? 'opacity-50 scale-95' : ''} ${!((groupBy === 'status' && getPermissions(task).canChangeStatus) || ((groupBy === 'project' || groupBy === 'priority') && getPermissions(task).canEditFields)) ? 'cursor-not-allowed opacity-80' : ''}`}
                                        >
                                            <div className="flex justify-between items-start mb-4">
                                                ...
                                                <div className="flex items-center gap-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 px-2 py-1 rounded-xl">
                                            ...
                                            <div className="flex flex-wrap gap-2 mb-5">
                                                <div className="flex items-center gap-2 px-2.5 py-1.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl">
                                                ...
                                                {task.tags?.map(tag => (
                                                    <span key={tag} className="px-2.5 py-1.5 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl text-zinc-600 dark:text-zinc-400 text-[9px] font-bold uppercase shadow-sm">#{tag}</span>
                                                ))}
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between pt-4 border-t dark:border-zinc-800/50">
                                                <div className="flex items-center gap-2">
                                                    {assignee ? (
                                                        <img src={assignee.avatar} className="w-8 h-8 rounded-full object-cover border border-zinc-200 dark:border-zinc-700 shadow-sm" title={assignee.name} alt={assignee.name} />
                                                    ) : (
                                                        <div className="w-8 h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 flex items-center justify-center text-[10px] font-bold text-zinc-400">?</div>
                                                    )}
                                                </div>
