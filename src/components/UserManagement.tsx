...
const UserManagement: React.FC<Props> = ({ 
    ...
    const handleUpdatePassword = async () => {
        if (!passwordTargetUser || !newDirectPassword) return;
        ...
        try {
            await adminService.updatePassword(passwordTargetUser.id, newDirectPassword);
            alert(`Пароль для ${passwordTargetUser.name} успешно изменен!`);
        ...
        <div className="animate-fade-in space-y-6 pb-20">
                                    ...
                                    <td className="p-3">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button 
                                                ...
                                                className="p-2 bg-zinc-50 dark:bg-zinc-900 rounded-xl hover:text-amber-600 transition-colors border dark:border-zinc-800"
                                            ...
                                            {user.id !== currentUser.id && (
                                                <button 
                                                    onClick={() => { if(window.confirm('ВНИМАНИЕ: Сотрудник будет полностью удален из систем Auth и базы данных. Продолжить?')) onDeleteUser(user.id); }} 
            ...
            {isPasswordModalOpen && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-3">
                    ...
                    <div className="relative clay-panel w-full max-w-sm shadow-2xl overflow-hidden animate-modal-in">
                        <div className="p-6">
                            <h3 className="text-xl font-black uppercase text-zinc-950 dark:text-white mb-2">Смена пароля</h3>
                            <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-6 border-b border-zinc-100 dark:border-zinc-800 pb-4">
                                Сотрудник: <span className="text-zinc-950 dark:text-white">{passwordTargetUser?.name}</span>
                            </p>
                            
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <label className="text-[10px] font-black text-zinc-400 uppercase tracking-widest ml-1">Новый пароль</label>
                                    <input 
                                        autoFocus
                                        type="text" 
                                        value={newDirectPassword}
                                        onChange={(e) => setNewDirectPassword(e.target.value)}
                                        className="w-full px-5 py-4 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl text-sm font-bold outline-none ring-primary/20 transition-all font-mono"
                                ...
                                <div className="flex gap-3 pt-4">
                                    <button 
                                        ...
                                        className="flex-1 py-3 text-xs font-black uppercase tracking-widest text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-xl transition-all"
                                    >
                                        Отмена
                                    </button>
                                    <button 
                                        ...
                                        className="flex-1 py-3 bg-zinc-950 dark:bg-white text-white dark:text-black text-xs font-black uppercase tracking-widest rounded-xl transition-all shadow-lg flex items-center justify-center gap-2"
                                    >
                                        {isUpdatingPassword ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Обновить'}
                                    </button>
                                </div>
                            </div>
                        </div>
