...
const RocketLoader: React.FC<{ text?: string }> = ({ text = "Загрузка системы..." }) => {
  return (
    <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-transparent overflow-hidden">
      ...
      <div className="mt-12 text-center premium-card p-8 bg-surface1/50 backdrop-blur-xl border-surface3 shadow-2xl">
        <h2 className="text-2xl font-black uppercase tracking-[0.4em] text-text animate-pulse font-headline">
          {text}
        </h2>
        <p className="text-[11px] font-black text-muted uppercase mt-4 tracking-[0.3em] font-headline opacity-60">
          Подготавливаем топливо для знаний
        </p>
        <div className="mt-8 w-48 h-1 bg-surface3 rounded-full overflow-hidden mx-auto">
            <motion.div 
                initial={{ x: '-100%' }}
                animate={{ x: '100%' }}
                transition={{ duration: 2, repeat: Infinity, ease: "linear" }}
                className="w-full h-full bg-red shadow-[0_0_15px_rgba(230,57,70,0.5)]"
            />
        </div>
      </div>
