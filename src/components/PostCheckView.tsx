            ...
            <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-4">
                <div className="flex gap-4">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm ${activeTab === 'input' ? 'bg-primary text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 border border-zinc-200 dark:border-zinc-700'}`}
                    >
                        <ListPlus size={16} /> Ввод данных
                    </button>
                    <button 
                        onClick={() => setActiveTab('analytics')}
                        className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm ${activeTab === 'analytics' ? 'bg-primary text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 border border-zinc-200 dark:border-zinc-700'}`}
                    >
                    ...
                    {canManage && (
                        <button 
                            ...
                            className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all shadow-sm ${activeTab === 'settings' ? 'bg-primary text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 border border-zinc-200 dark:border-zinc-700'}`}
                        >
            ...
            <div className="relative">
                {activeTab === 'input' && (
                    <>
                        ...
                        />
                        {showForm && (
                            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3 animate-fade-in">
                                <EntryForm 
                                    user={user} 
