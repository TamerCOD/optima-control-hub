...
const DutiesView: React.FC<Props> = ({ currentUser, roles }) => {
    ...
    const handleGenerateSchedule = async (targetMonth: Date) => {
        ...
        const existingForMonth = duties.filter(d => d.date.startsWith(format(targetMonth, 'yyyy-MM')));
        ...
        days.forEach(day => {
            ...
            settings.types.forEach(dutyType => {
                ...
                while (!assigned && attempts < eligibleUserIds.length) {
                    const userId = eligibleUserIds[currentIndex % eligibleUserIds.length];
                    
                    // Check if user has a day off on this date
                    const hasDayOff = daysOff.some(d => d.date === dateStr && d.userId === userId);
                    ...
                    if (!hasDayOff) {
                        ...
                        assigned = true;
                        lastAssignedIndex[dutyType.id] = currentIndex;
                    }
                    currentIndex++;
                    attempts++;
                }
        ...
        try {
            ...
            setDuties(prev => [
                ...prev.filter(d => !d.date.startsWith(format(targetMonth, 'yyyy-MM')) || d.dutyTypeId === 'DAY_OFF'),
                ...newDuties
            ]);
                        ...
                        <thead>
                            <tr>
                                ...
                                {daysInMonth.map(day => {
                                    ...
                                    return (
                                        <th key={day.toString()} className={`p-2 text-center text-xs border-b border-r border-zinc-200 dark:border-zinc-700/50 min-w-[40px] ${isWeekend ? 'bg-red-50 dark:bg-red-900/20 text-red-500' : 'bg-zinc-100 dark:bg-zinc-800/80'}`}>
                                                    ...
                                                    {dayDuties.filter(d => d.dutyTypeId !== 'DAY_OFF').map(dayDuty => {
                                                        ...
                                                        return (
                                                            <div 
                                                                ...
                                                                className="text-[10px] font-bold text-white rounded p-1 truncate shadow-sm" 
            ...
            {activeTab === 'CALENDAR' && (
                <div>
                    <div className="flex items-center justify-between mb-6 bg-zinc-100 dark:bg-zinc-800/50 p-3 rounded-2xl">
                        <button onClick={() => setBaseMonth(subMonths(baseMonth, 3))} className="p-2 hover:bg-white dark:hover:bg-zinc-700 rounded-xl transition-colors flex items-center gap-2 text-sm font-bold uppercase tracking-widest">
                            <ChevronLeft size={20} /> Пред. 3 месяца
                        </button>
                        <h2 className="text-lg font-black uppercase tracking-widest text-center">
                            {format(subMonths(baseMonth, 1), 'LLL yyyy', { locale: ru })} — {format(addMonths(baseMonth, 1), 'LLL yyyy', { locale: ru })}
                        </h2>
                        <button onClick={() => setBaseMonth(addMonths(baseMonth, 3))} className="p-2 hover:bg-white dark:hover:bg-zinc-700 rounded-xl transition-colors flex items-center gap-2 text-sm font-bold uppercase tracking-widest">
                            След. 3 месяца <ChevronRight size={20} />
                        </button>
                    </div>
                    <div className="space-y-8">
                        {[subMonths(baseMonth, 1), baseMonth, addMonths(baseMonth, 1)].map(m => (
                            <div key={m.toString()}>
                                {renderCalendar(m)}
                            </div>
                        ))}
