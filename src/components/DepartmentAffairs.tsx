...
const DepartmentAffairs: React.FC<Props> = ({ currentUser, roles }) => {
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            {/* Tabs */}
            <div className="flex gap-3 border-b border-zinc-200 dark:border-zinc-800 overflow-x-auto pb-4 custom-scrollbar">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-6 py-3 text-[10px] font-bold uppercase tracking-widest transition-all rounded-2xl shadow-sm hover:scale-[1.02] ${
                            activeTab === tab.id
                                ...
                                : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 border border-zinc-200 dark:border-zinc-700'
                        }`}
                    >
                        {tab.icon}
                        {tab.label}
            ...
            {/* Content */}
            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-[2.5rem] shadow-sm min-h-[500px] p-2 md:p-3">
